...
#module for private subnet #1
module "priv_subnet_1" {
  source = "./subnet"
  vpc_id = module.vpc.vpc
  subnet-cidr = var.priv-subnet1-cidr
  subnet-name = var.priv-subnet1-name
  map_public_ip_on_launch = false
  availability_zone = var.priv_subnet_1_availability_zone
}

#module for private subnet #2
module "priv_subnet_2" {
  source = "./subnet"
  vpc_id = module.vpc.vpc
  subnet-cidr = var.priv-subnet2-cidr
  subnet-name = var.priv-subnet2-name
  map_public_ip_on_launch = false
  availability_zone = var.priv_subnet_2_availability_zone
}
...
#module for route table association for connecting priv_subnet_1 with private-route
 module "priv-rta-1" {
   source = "./routetable-association"
   subnet-id =  module.priv_subnet_1.subnet-id
   route-table-id = module.private-route.routetable
 } 


#module for route table association for connecting priv_subnet_2 with private-route
 module "priv-rta-2" {
   source = "./routetable-association"
   subnet-id =  module.priv_subnet_2.subnet-id
   route-table-id = module.private-route.routetable
 } 
...
module "autoscaling" {
  ...
  iam_instance_profile = module.iam-role.iam-instance-profile
  ...
  vpc_zone_identifier = [module.priv_subnet_1.subnet-id]
...
#module for Relational DataBase Insatnce
module "rds" {
  source = "./relational-db-instance"
  rds_sg_name = var.rds_sg_name
  vpc_id = module.vpc.vpc
  private_subnet_ids = [module.priv_subnet_1.subnet-id, module.priv_subnet_2.subnet-id]
  db_name = var.db_name
